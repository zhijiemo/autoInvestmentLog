...
df_stocks = pd.read_excel(localtime + '/A_data.xls', dtype='object')[['code', 'name', 'trade', 'changepercent']]
...
df_inner_goods = pd.read_excel(localtime + '/Future_C_data.xls', dtype='object')[['symbol', 'current_price', 'last_settle_price']]
...
df_inner_finance = pd.read_excel(localtime + '/Future_F_data.xls', dtype='object')[['symbol', 'current_price', 'open']]
...
df_outer = pd.read_excel(localtime + '/Future_W_data.xls', dtype='object')[['symbol', 'current_price', 'last_settle_price']]
...
for t in shibor_time_list:
    try:
        df_temp = pd.read_excel(localtime + '/Shibor_' + t + '.xls', index_col=0)
        rate = df_temp['利率(%)'][0]
        bp = df_temp['涨跌(BP)'][0]
...
for t in libor_time_list:
    try:
        df_temp = pd.read_excel(localtime + '/Libor_' + t + '.xls', index_col=0)
        rate = df_temp['利率(%)'][0]
        bp = df_temp['涨跌(BP)'][0]
